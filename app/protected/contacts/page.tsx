      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            ...
            <span className={`rounded-full px-3 py-1 text-xs font-semibold ${statusBadgeClass(contact.status)}`}>
              {contact.status}
            </span>
          </div>
        </div>
              ...
              >
                <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-slate-500">Rôle : {deal.roleInDeal}</p>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    <span className={`rounded-full px-3 py-1 text-xs font-semibold ${dealStatusBadgeClass(deal.statusInDeal)}`}>
                      {deal.statusInDeal}
                    </span>
                    <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-700">
                ...
                <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                  <div className="rounded-xl bg-slate-50 p-3">
                    <p className="text-xs uppercase tracking-wide text-slate-500">Date de contact</p>
...
async function ContactsContent() {
  const { allContacts, activeContactsCount, linkedDealsCount } = await getContactsView();
      ...
      <div className="mx-auto max-w-7xl">
        <div className="mb-8 flex items-center justify-between gap-4">
          <div>
            <p className="text-sm font-medium text-slate-500">Module CRM</p>
            <h1 className="mt-1 text-3xl font-bold tracking-tight">Contacts</h1>
            <p className="mt-2 text-sm text-slate-500">Base contacts connectée à Supabase</p>
          </div>
          <a
