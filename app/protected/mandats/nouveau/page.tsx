...
export default function NouveauMandatPage() {
  ...
  const [form, setForm] = useState({
    ...
    retainer_monthly: "", estimated_fee_amount: "", operation_amount: "", notes: "",
  });

  useEffect(() => {
    getAllOrganisationsSimple().then(setOrgs).catch(() => {});
  }, []);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    router.push(`/protected/mandats/${r.id}`);
  }
  ...
  return (
    <div style={{ padding: "28px 24px", minHeight: "100vh", background: "var(--bg)" }}>
      <div style={{ maxWidth: 680, margin: "0 auto" }}>
        ...
        <Link href="/protected/mandats" style={{ display: "inline-flex", alignItems: "center", gap: 5, fontSize: 12.5, color: "var(--text-4)", textDecoration: "none", marginBottom: 20 }}>
          <ArrowLeft size={13} /> Mandats
        </Link>

        <h1 style={{ margin: "0 0 20px", fontSize: 22, fontWeight: 800, color: "var(--text-1)" }}>Nouveau mandat</h1>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={section}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              <div>
                ...
                <input style={inp} placeholder="ex: Mandat M&A Société Dupont" value={form.name} onChange={setF("name")} />
              </div>
              ...
              <div>
                ...
                <textarea style={{ ...inp, minHeight: 70, resize: "vertical" }} placeholder="Contexte de la mission…" value={form.description} onChange={setF("description")} />
              </div>
            </div>
          </div>
          ...
          <div style={section}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              <div>
                <label style={lbl}>
                  {{ma_sell:"Valorisation cible (EV)",ma_buy:"Budget d'acquisition",cfo_advisor:"Budget mission"}[form.type] ?? "Montant de l'opération"}
                </label>
          ...
          <div style={section}>
            ...
            <textarea style={{ ...inp, minHeight: 80, resize: "vertical" }} placeholder="Notes internes…" value={form.notes} onChange={setF("notes")} />
          </div>

          {/* Actions */}
          <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
            <Link href="/protected/mandats" style={{
