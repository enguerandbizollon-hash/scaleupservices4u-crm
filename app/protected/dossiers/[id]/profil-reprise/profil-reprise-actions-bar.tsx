"use client";
// Barre d'actions du profil de reprise : générer/régénérer (IA) et imprimer
// en PDF. Masquée à l'impression.

import { useState } from "react";
import { useRouter } from "next/navigation";
import { generateProfilReprise } from "@/actions/livrables";

export function ProfilRepriseActionsBar({ dealId, hasContent }: { dealId: string; hasContent: boolean }) {
  ...
  async function handleGenerate() {
    ...
    setBusy(true);
    setError(null);
    const res = await generateProfilReprise(dealId);
    setBusy(false);
  ...
  return (
    <div className="no-print" style={{
      ...
      <button onClick={handleGenerate} disabled={busy}
        ...
        {busy ? "Génération en cours..." : hasContent ? "Régénérer le profil" : "Générer le profil"}
      </button>
      {hasContent && (
      ...
      <span style={{ fontSize: 12, color: "rgba(255,255,255,.55)" }}>
        Document anonymisé : le nom du repreneur est retiré par le code après génération. Relisez avant tout envoi.
      </span>
