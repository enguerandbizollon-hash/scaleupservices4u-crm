...
async function Content({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: deal, error: dealError } = await supabase
    .from("deals")
    .select("id,name,deal_type,deal_status,deal_stage,priority_level,sector,location,description,start_date,target_date,target_amount,committed_amount,closed_amount,currency,company_stage,company_geography,mandate_id,pre_money_valuation,post_money_valuation,round_type,runway_months,use_of_funds,current_investors,asking_price_min,asking_price_max,partial_sale_ok,management_retention,deal_timing,ai_financial_score,ai_valuation_low,ai_valuation_high,target_sectors,target_geographies,target_revenue_min,target_revenue_max,target_ev_min,target_ev_max,acquisition_budget_min,acquisition_budget_max,full_acquisition_required,strategic_rationale,target_stage")
