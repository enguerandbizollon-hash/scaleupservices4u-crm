...
async function AgendaContent() {
  ...
  const [
    ...
    supabase.from("agenda_events").select("id,title,event_type,starts_at,ends_at,location,description,status,deal_id").order("starts_at", { ascending: true }),
    supabase.from("tasks").select("id,title,description,task_status,priority_level,due_date,deal_id,contact_id").order("due_date", { ascending: true }),
    supabase.from("deals").select("id,name"),
  ]);

  const dealsMap: Record<string, string> = Object.fromEntries(
    ...
    <div className="p-8">
      <div className="mb-8 flex items-center justify-between">
        ...
        <div className="flex gap-3">
          <Link
            href="/protected/agenda/nouvelle-tache"
            className="rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50"
          >
            + Tâche
          </Link>
          <Link
            href="/protected/agenda/nouvel-evenement"
                ...
                <div key={event.id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      {event.deal_id && (
                        <p className="mt-0.5 text-xs text-slate-500">{dealsMap[event.deal_id] ?? "—"}</p>
                      )}
                    </div>
                  ...
                  {event.description && (
                    <p className="mt-3 text-xs text-slate-500">{event.description}</p>
                  )}
                </div>
              ))
            )}
                ...
                <div key={task.id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      {task.deal_id && (
                        <p className="mt-0.5 text-xs text-slate-500">{dealsMap[task.deal_id] ?? "—"}</p>
                      )}
                    </div>
                  ...
                  <div className="mt-3 rounded-xl bg-slate-50 p-2.5">
                    ...
                    <p className="mt-0.5 text-sm font-medium">{formatDateOnly(task.due_date)}</p>
                  </div>
                </div>
              ))
            )}
